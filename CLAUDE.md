...
- Pre-empirical engineering may consume at most 20% of the stage cap. Reforecast wall-time and
  storage after the first 1% of planned throughput. A projection above 2x the original or any
  wall-time/storage/throughput budget exceed requires a safe-boundary pause and decision packet.
  Runtime alone never permits optional stopping, but a frozen budget exceed is a gate. Unless an
  approved design says otherwise, transient storage is capped at 5 GiB.
- For the same scientific gate/lineage, all consumed wall-time, CPU/GPU time, storage,
  throughput samples, and pre-empirical engineering budget accumulate across generations,
  successors, replacement roles, process restarts, and new run roots. None may reset unless a
  human explicitly approves new authority/contract; that contract must record prior
  consumption, carry-over, and the expressly approved reset boundary.
- Resource telemetry is an operational safety and planning control, not a scientific outcome or
  claim criterion unless the approved design explicitly makes resource use the measured
  quantity. A resource-accounting defect is blocking only when direct evidence shows or
  materially indicates a frozen cap exceed, unsafe continuation, label-dependent
  stopping/selection, an incomplete required workload, or corrupted/unverifiable scientific
  evidence. When a fixed workload completed, its schedule and evidence chain verify, the
  headline result independently reproduces, no label-driven stopping occurred, and there is no
  direct evidence of a cap exceed, missing fine-grained or parent-process accounting is a
  non-blocking technical caveat. Record it and improve telemetry prospectively; do not rerun
  outcome-bearing work solely to perfect resource accounting. This rule never creates or
  changes a scientific outcome/edge, and it cannot unlock a dependent gate whose scientific
  edge is absent.
