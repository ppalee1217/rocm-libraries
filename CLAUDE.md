...
- Keep three units separate:
  ...
  - An `execution_tranche` may share Main-agent planning context, environment/setup, run root, and
    repair ledger; each gate retains its own plans, fresh implementer, and fresh verifier.
...
- Track repair rounds as append-only provenance for every R0-R3 gate, but never use the
  accumulated count as a stop, approval, or completion gate. Reaching two consecutive rounds
