...
- Treat Cursor Markdown preview and navigation as the equivalent behavior in the current
  Codex client.
- When synchronizing the canonical roadmap to its mirrors, use a file-edit or copy mechanism
  allowed by the current Codex session, preserve unrelated changes, and finish with the
  workflow's required `diff` checks.
