...
- Use the file-edit mechanism allowed by the current Codex session, preserve unrelated
  changes, and follow the workflow's canonical/mirror rules.
...
3. 把 template 的「今日目標」與「對應 roadmap item」預填好（這是事實，可代填），其餘留空白給學習者。
   - **「對應 roadmap item」必須逐項鏡射當天 roadmap 小節的「全部」item，一項都不能少**：包含每個主 item、其下所有子項與 `✅ 完成判準`，以及**選做（（AMD 資源，選做））與建議／選讀（（全程指引，建議）、（選讀／按需））項目**。
   - 選做／建議／選讀項目**照抄但保留其標記**（如 `（AMD 資源，選做）`、`（建議）`、`（選讀／按需）`），讓學習者一眼看出哪些可略過；**但絕不能因為是選做就整條省略**。
   - 沿用 roadmap 的勾選狀態：已完成用 `- [x]`、未完成用 `- [ ]`；不確定就維持 `- [ ]`。
   - **不只列 item 文字——roadmap 為該 item 標的 `📚 參考資源`（doc / code / asm 連結）也要一併帶入**，採「雙軌」：在該 item 下加一行 inline `📚 參考資源：...`，同時在底部「code & doc 參考」段落彙整同一批連結。只連已存在的檔（見下節）。漏參考資源＝學習者不知去哪查。
   - 理由：學習者只看筆記決定當天要做什麼，**筆記漏列＝他當天漏做**，這是本 skill 最重要的正確性要求。
...
筆記檔名符合 `MMDD-{Description}.md`；template 章節齊全；**「對應 roadmap item」逐項對得上當天 roadmap
小節、數量與標記一致（主項＋子項＋選做／建議／選讀全數列出，選做/建議保留標記，一項都不漏）**；
**每個 item 的 `📚 參考資源` 與 roadmap 的 📚 對得上（inline 於 item 下 + 底部段落彙整皆具）**；
AI 補的每個連結路徑/行號都經查證（只連已存在的檔）；AI 協助格式化處遵守可讀性規範
（nested bullet 切割、長段斷點）；學習內容欄位保持由學習者填寫（AI 未代寫）。
