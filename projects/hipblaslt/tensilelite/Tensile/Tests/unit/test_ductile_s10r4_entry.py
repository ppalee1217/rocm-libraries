...
def test_contract_identity_and_fixture_hash():
    ...
    assert contract["contract_id"] == "S10R4-A47-EXACT-FRAME-OPERATIONAL-ENTRY-V1"
...
def mapping_rows(pass_id="A", hashes=("h0",)):
    ...
    sizes = {
        ...
        for row in load_json(PROTOCOL_ROOT / "manifests/s10r4-size-registry.json")["sizes"]
    }
...
def test_post_binding_activation_checks_exact_nineteen_path_git_commit(tmp_path, monkeypatch):
    ...
    absence = {"durable": [], "raw_scopes": [], "lock": {"path": lock_relative, "absent": False}, "projection_sha256": "d" * 64}
    monkeypatch.setattr(contract_module, "_absence_observation", lambda *_args, **_kwargs: absence)
...
def test_actual_constructor_attaches_non_circular_four_event_governance(
    ...
    plan_a_relative = "agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame/gates/s10r4/plan_a.md"
    plan_a_path = copy_relative(plan_a_relative)
    revision_path = copy_relative(
        "agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame/gates/s10r4/plan_a-revisions.jsonl"
    )
    baseline_path = copy_relative(
        "agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame/baseline.md"
    )
    report_md = copy_relative(
        "agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame/gates/s10r4/implementer-report.md"
    )
    report_json = copy_relative(
        "agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame/gates/s10r4/implementer-report.json"
    )
    reported_resources = load_json(report_json)["resources"]
    ...
    pinned_root = repo / "agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame/artifacts/prelabel-build/pinned-source"
    pinned_records = []
    ...
    provisional = {
        ...
        "document_kind": "prebinding_audit_ledger", "events": make_events(3),
        "ledger_digest": "0" * 64,
    }
    provisional["ledger_digest"] = canonical_sha256(
        {key: value for key, value in provisional.items() if key != "ledger_digest"}
    )
    ledger_path.write_bytes(canonical_json(provisional, newline=True))
    ...
    events[3]["inputs"]["lock_input_projection_hashes"] = {
        ...
        for key in (
            ...
            "source_realization", "governance_base",
        )
    }
    ...
    ledger_path.write_bytes(canonical_json(ledger, newline=True))
    ...
    assert lock["governance_and_boundary"]["base"]["resources"] == {
        ...
        "implementation_reported": reported_resources,
