...
class DurableInterruption(RuntimeError):
    """Signals that a full-horizon run stopped after a durable checkpoint."""

    def __init__(self, generation):
        super().__init__(f"durable interruption after generation {generation}")
...
class GeneticAlgorithm:
    ...
    def __init__(self,
                 ...
                 interrupt_after_generation: int = None):
    ...
    def optimize(self):
        ...
        try:
            for gen in range(gen_start, self.n_gen + 1):
                configs = self.space.transform(pop)
                if self.observer is not None:
                ...
                if scores.ndim == 1:
                    scores = scores[None, ...]
                if self.observer is not None:
                    self._emit("fitness_matrix", gen, {
                        "dtype": scores.dtype.str,
                        "shape": list(scores.shape),
                        "values": scores.tolist(),
                        "data_sha256": hashlib.sha256(scores.tobytes(order="C")).hexdigest(),
                    })
                ...
                if self.observer is not None:
                    ...
                    self._emit("generation_stats", gen, {
                        ...
                        "per_gene_diversity": [
                            {"gene": name, "value": float(value)}
                            for name, value in per_gene_diversity.items()
                        ],
                ...
                if self.observer is not None:
                    self._emit("termination_updated", gen, {
                        "next_population_size": int(self.pop_size),
                        "decay_mode": self._decay_type,
                    })
        ...
        if self.termination_reason is None:
            self.termination_reason = "fixed_horizon"
            if self.observer is not None:
                self._emit("termination", self.final_generation, {
                    "reason": self.termination_reason,
                    "detail": "configured generation horizon reached",
                })
        ...
        if self.observer is not None:
            self._emit("final_champion", self.final_generation, {
                "configs": X,
                "fitness": best.F.tolist(),
                "fitness_dtype": best.F.dtype.str,
                "fitness_sha256": hashlib.sha256(best.F.tobytes(order="C")).hexdigest(),
            })
