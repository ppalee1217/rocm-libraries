...
def _pandas():
    try:
        ...
        raise RuntimeError("this Population helper requires pandas") from exc
    return pandas


def _hamming_mean(values):
    values = np.asarray(values)
    if values.shape[0] < 2:
        return float("nan")
    left, right = np.triu_indices(values.shape[0], 1)
    return np.mean(values[left] != values[right], axis=0)
...
class Population(np.ndarray, Sequence):
    ...
    def nunique(self, key: str = None) -> Union[object, int]:
        if key:
    ...
    def diversity(self, reduce=True, metric="hamming"):
        if self.size == 0:
            return 0
        if metric != "hamming":
            raise RuntimeError("non-Hamming diversity requires scipy and is unavailable in M00")

        X = self.ary
        if reduce:
            return float(np.mean(_hamming_mean(X)))

        div = [float(np.mean(_hamming_mean(x[:, None]))) for x in X.T]
