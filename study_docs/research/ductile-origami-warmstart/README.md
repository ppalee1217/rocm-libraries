...
>
> **2026-08-09 `PER-SHAPE-SOO-OUTCOME-20260809`(user-approved):**S14 內部設計改為 **per-shape single-objective**(每個 shape 各一條 GA、per-shape guidance、per-shape 評估;medium+large confirmatory、tiny 探索;n_gen=30+原生早停+gen-10 檢查點;5 對全新 seed 24001–24005;≤6 GPUs 非-0;雙向效應估計、per-shape directional-consistency gate;guidance 採 entropy-cap(`ENTROPY-CAP-20260810`);claim 依 charter **§8.6a `CLAIM-SCOPE-S14-20260810`**(owner-approved:two-sided,資料支持時得宣稱受測 shape 最終品質提升,modest power、不一般化、不預設結論))。criterion `S14_GUIDED_OUTCOME_POSITIVE` → `S14_PER_SHAPE_OUTCOME`;S11→S14 edge 與 S12+ freeze 不變。詳細權威 = S14 design **§10–§11**(舊 joint-MOO 設計保留於 §1–§9)。
>
...
| S14 | Stage 1 full-GA baseline-vs-guided real-GPU outcome（RESCOPE-STAGE1-OUTCOME-20260807；active,two-reviewer AGREE 2026-08-07;pins: P0-cap=512（user-authorized 2026-08-07,was 64）／5 seeds 4/5（user-authorized 2026-08-07,was 3/3）／G-F,S optional-reserved;native early-stop kept(n_gen=30 max);champion-primary,AUC secondary;5-seed 可平行(5 卡);pre-seal preflight required）**⚠️ 內部設計 SUPERSEDED 2026-08-09 by `PER-SHAPE-SOO-OUTCOME-20260809`:per-shape SOO(每 shape 一條 GA)、medium+large confirmatory/tiny 探索、n_gen=30+原生早停+gen-10 檢查點、per-shape guidance、5 對全新 seed 24001–24005、≤6 GPUs 非-0、雙向估計、criterion `S14_PER_SHAPE_OUTCOME`;S11→S14 edge 不變。詳見 design §10**| `approved` | `gated` | `DESIGN_APPROVED` | `not_evaluated` | `absent` | `reports/staged/full-ga-baseline-vs-guided-outcome-report.md` |
