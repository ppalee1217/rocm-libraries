#!/usr/bin/env python3
"""P17 -- large: the *difference* between the two arms' best-so-far curves.

Why a difference and not two curves
-----------------------------------
A capped large run climbs a median ~29 % from Gen0 to its end, while the two
arms finish a median ~0.4 % apart -- an even more lopsided ratio than medium's
~29 % against ~1 %.  Two absolute curves that close together, drawn on that
y-range, are one line, and shading between them is a hairline.  So each panel
plots
...
On **large capped this is not a rounding detail**: the two arms early-stop at
very different generations (21 vs 30 on three of the five seeds), so B* throws
away up to ~24 % of the longer arm's evaluations -- seed 24005 loses 23.6 % of
arm G, 24004 loses 20.7 % of arm G, 24003 loses 20.2 % of arm F.  A fifth to a
quarter of one arm's search is simply not scored on those seeds.  No other page
in the deck states this, which is why every panel prints its own figure.

Run on the HOST:  python3 p17_evolution_large.py
"""
...
def main():
    ...
    common.provenance(fig, "trajectory.jsonl · best_gflops_so_far vs "
                           ...
                           "+ stage5_native_{baseline,guided} · large · "
                           "5 paired seeds")
