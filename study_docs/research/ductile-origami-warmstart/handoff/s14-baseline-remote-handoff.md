...
- **benchmark measurement — per-size rotating policy (user-authorized 2026-08-07, Option C):** `NumWarmups = 321`, `EnqueuesPerSync = 321`. Rotating buffers are set **per size**: `(8,8,1,128)` and `(256,256,1,1024)` use **`RotatingBufferSize = 4096` MB (rotating ON, realistic cold-cache GFLOPS)**; the large size `(2304,1024,1,214336)` uses **rotating OFF (`RotatingBufferSize = 0`)** — its ~1.34GB working set far exceeds GPU cache (rotating adds ~nothing / no meaningful inflation) and the client's rotating check signed-overflows for it at any practical RBS. If the client cannot set RBS per-size, achieve this by measuring the 2 small/medium sizes @RBS=4096 and the large size @RBS=0 in separate passes, then merging by canonical config identity so every candidate has a GFLOPS for all 3 sizes (Q=max_s over the 3 sizes unchanged).
