...
PROTOCOL_ID = "S10R4-A47-EXACT-FRAME-OPERATIONAL-ENTRY-V1"
...
def load_contract(path: Path = CONTRACT_PATH) -> dict[str, Any]:
    ...
    if contract.get("contract_id") != PROTOCOL_ID:
        raise S10R4Error("S10R4 contract identity mismatch")
...
COMMAND_TEMPLATES = {
    ...
    "prepare_prelabel": [
        ...
        "--build-root", "agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame/artifacts/prelabel-build",
        "--fixture-root", "agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame/artifacts/prelabel-fixtures",
    ...
    "build-lock": [
        ...
        "study_docs/research/ductile-origami-warmstart/protocol/v1/run_s10r4_entry.py",
        "build-lock", "--output",
        "study_docs/research/ductile-origami-warmstart/protocol/v1/locks/s10r4-stage1-exact-frame-entry-lock.json",
    ],
    "census_child": [
        "/opt/venv/bin/python3",
...
COMMAND_TEMPLATES["reproduction"] = [
    ...
    "study_docs/research/ductile-origami-warmstart/protocol/v1/locks/s10r4-stage1-exact-frame-entry-lock.json",
    "--output",
    "agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame/artifacts/reproduction/s10r4-independent-result.json",
]
...
COMMAND_TEMPLATES.update(
    {
        ...
        "native_configure": [
            ...
            "/src/rocm-libraries/agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame/artifacts/prelabel-build/native/source",
            "-B", "/src/rocm-libraries/agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame/artifacts/prelabel-build/native/build",
            "-DCMAKE_BUILD_TYPE=Release", "-DCMAKE_CXX_COMPILER=/opt/rocm/bin/amdclang++",
            "-DCMAKE_EXPORT_COMPILE_COMMANDS=ON",
        ],
        "native_build": [
            "/opt/venv/bin/cmake", "--build",
            "/src/rocm-libraries/agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame/artifacts/prelabel-build/native/build",
...
PLAN_A_REVISION = 12
PLAN_A_RAW_SHA256 = "c2d8b709f64d80bbb0b3a169397d689ec2e6faf7d974723d6d8688938bcda4d1"
PLAN_A_SIZE_BYTES = 91820
PLAN_A_PREDECESSOR = {
    "revision": 11,
    "raw_sha256": "3b1105d2d2b1a3dbf264ee0d4425cf2dd54e452e5348a3fcb15df5ee59db5a57",
    "size_bytes": 89134,
}
PHASE1_COMMIT_OID = "9e10067d6ea7ae6b979cf99d97ebee007d1ceec6"
PREBINDING_LEDGER_PATH = REPO_ROOT / "agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame/gates/s10r4/audit-verdict.json"
POST_COMMIT_LEDGER_PATH = REPO_ROOT / "agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame/closure/post-commit-audit.json"
PLAN_A_REVISION_LEDGER_PATH = REPO_ROOT / "agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame/gates/s10r4/plan_a-revisions.jsonl"
BASELINE_PATH = REPO_ROOT / "agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame/baseline.md"
IMPLEMENTER_REPORT_PATHS = (
    REPO_ROOT / "agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame/gates/s10r4/implementer-report.md",
    REPO_ROOT / "agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame/gates/s10r4/implementer-report.json",
)
...
def _source_realization(contract: Mapping[str, Any], native_build_manifest: Path) -> dict[str, Any]:
    pinned_root = REPO_ROOT / "agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame/artifacts/prelabel-build/pinned-source"
    ...
    prelabel_summary_path = REPO_ROOT / "agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame/artifacts/prelabel-fixtures/prelabel-build-summary.json"
...
def _selected_revision_record() -> dict[str, Any]:
    ...
    for raw_line in PLAN_A_REVISION_LEDGER_PATH.read_bytes().splitlines():
        ...
        if record.get("revision") == PLAN_A_REVISION:
            if selected is not None:
                raise S10R4Error("Plan-A revision ledger has duplicate selected revision")
            selected = record
    if selected is None:
        raise S10R4Error(f"Plan-A revision ledger lacks revision {PLAN_A_REVISION}")
    expected = {
...
def _governance_base(
    ...
    role_records = [
        {"role": "main", "task_id": "/root", "fresh": False, "implementation_participation": False},
        {
        ...
        },
        {
            ...
            "fresh": True, "implementation_participation": True,
        },
        {"role": "verifier", "task_id": None, "fresh": True, "implementation_participation": False},
    ]
    resources = {
        "planning_policy": contract["resources"],
        "implementation_reported": report.get("resources", "UNKNOWN"),
        "whole_role_cpu_seconds": report.get("resources", {}).get("whole_role_cpu_seconds", "UNKNOWN") if isinstance(report.get("resources"), dict) else "UNKNOWN",
        "whole_role_wall_seconds": report.get("resources", {}).get("whole_role_wall_seconds", "UNKNOWN") if isinstance(report.get("resources"), dict) else "UNKNOWN",
    }
    base = {
        "risk_tier": "R3",
        "repair_numeric_stop_cap": None,
        "role_records": role_records,
        "fresh_role_thread_count": 5,
        "resources": resources,
...
def _validate_prebinding_ledger_inputs(
    ...
    *,
    ...
    contract_record = _binding_record(CONTRACT_PATH, canonical=contract)
    ...
    if plan_a_record["raw_sha256"] != PLAN_A_RAW_SHA256 or plan_a_record["size_bytes"] != PLAN_A_SIZE_BYTES:
        raise S10R4Error("current Plan-A revision 12 identity mismatch")
...
def _construct_effective_lock_v11(
    *,
    ...
    if audit_verdict_path.resolve(strict=True) != PREBINDING_LEDGER_PATH.resolve(strict=True):
        raise S10R4Error("effective lock must use the canonical cumulative prebinding audit ledger")
    if (
    ...
    lifecycle = {
        ...
        "activation": {
            ...
            "absence_rule": "all_formal_outcome_and_raw_scopes_absent",
        },
        "post_audit": {
    ...
    validate_effective_lock(lock, remeasure=False)
...
def validate_effective_lock(lock: Mapping[str, Any], *, remeasure: bool = True) -> None:
    return _validate_effective_lock_v11(lock, remeasure=remeasure)
...
def _validate_lock_source_shapes(source: Mapping[str, Any]) -> None:
    ...
    pinned_root = (
        REPO_ROOT
        / "agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame/artifacts/prelabel-build/pinned-source"
...
def _validate_effective_lock_v11(lock: Mapping[str, Any], *, remeasure: bool) -> None:
    ...
    section_keys: dict[str, set[str]] = {
        "phase1": {
            "contract_record", "phase1_contract_commit_oid", "phase1_post_commit_audit", "authority_records",
    ...
    require_exact_keys(
        ...
        {"materialize", "tool_identity", "native_build", "formal_child"},
    ...
    require_exact_keys(
        base,
        {
            "risk_tier", "repair_numeric_stop_cap", "role_records", "fresh_role_thread_count",
    ...
    if (
        ...
        != _binding_record(PREBINDING_LEDGER_PATH)
    ...
    if (
        ...
        or lock["lock_id"] != "S10R4-A47-EXACT-FRAME-EFFECTIVE-BINDING-V1"
    ...
    contract = load_contract()
    if lock["phase1"]["contract_record"] != _binding_record(CONTRACT_PATH, canonical=contract):
    ...
    if base["risk_tier"] != "R3" or base["repair_numeric_stop_cap"] is not None:
...
def _verify_committed_effective_lock_v11(path: Path) -> dict[str, Any]:
    ...
    if inputs["formal_absence"] != _absence_observation(contract, include_lock=False):
        raise S10R4Error("post-binding formal/outcome/raw-scope absence mismatch")
    if (
