...
RUN_ROOT = REPO_ROOT / "agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame-binding-02"
...
LOCK_PATH = REPO_ROOT / "study_docs/research/ductile-origami-warmstart/protocol/v1/locks/s10r4-stage1-exact-frame-entry-binding-02-lock.json"
...
WORKER_PYTHONPATH = "/src/rocm-libraries/agent_run/260802-ductile-factorized-guidance-s10r4-exact-frame-binding-02/artifacts/prelabel-build/pinned-source/projects/hipblaslt/tensilelite"
...
INTERNAL_COMMANDS = frozenset(
    {"_census-child", "_mapping-child", "_correctness-build-child"}
)


def _require_top_level_runtime() -> None:
...
def command_validate_contract(_: argparse.Namespace) -> None:
    ...
    supplement_path = SCRIPT_PATH.parent / "s10r4-stage1-exact-frame-entry-binding-02-contract.yaml"
    supplement = load_contract_supplement(supplement_path)
    amendment = load_prebinding_recovery_amendment()
    retired = amendment["retired_prebinding_attempt"]
    successor = amendment["successor_prebinding_ledger"]
    _print(
        {
            "checkpoint_id": "S10R4",
            "binding_id": supplement["binding_id"],
            "contract_id": contract["contract_id"],
            "base_contract_raw_sha256": raw_sha256(SCRIPT_PATH.parent / "s10r4-stage1-exact-frame-entry-contract.yaml"),
            "supplement_raw_sha256": raw_sha256(supplement_path),
            "amendment_raw_sha256": AMENDMENT_RAW_SHA256,
            "amendment_canonical_sha256": AMENDMENT_CANONICAL_SHA256,
            "amendment_commit_oid": AMENDMENT_COMMIT_OID,
            "retired_prebinding_attempt": {
                "path": retired["path"],
                "raw_sha256": retired["raw_sha256"],
                "canonical_sha256": retired["canonical_sha256"],
                "state": retired["state"],
                "seal_authority": retired["seal_authority"],
            },
            "active_prebinding_ledger": {
                "path": successor["path"],
                "role": successor["role"],
                "present": (REPO_ROOT / successor["path"]).exists(),
            },
...
def command_build_lock(arguments: argparse.Namespace) -> None:
    ...
    lock = construct_effective_lock(
        ...
        audit_verdict_path=RUN_ROOT / "gates/s10r4/audit-verdict-r2.json",
...
def _admit_runtime(arguments: argparse.Namespace) -> None:
    """Apply the command-specific first runtime guard after exact parsing."""
    if arguments.command not in INTERNAL_COMMANDS:
        _require_top_level_runtime()
        _admit_current_argv(arguments)
        return
    request_argument = Path(arguments.request)
    result_argument = Path(arguments.result)
    if not request_argument.is_absolute() or not result_argument.is_absolute():
        raise S10R4Error("formal worker request/result paths must be absolute")
    request_path = request_argument.resolve(strict=True)
    result_path = result_argument.resolve(strict=False)
    ...
    if Path.cwd().resolve(strict=True) != request_path.parent:
        raise S10R4Error("formal worker cwd must equal the exact request parent")
    _admit_current_argv(arguments)
    ...
    if arguments.command in ("_census-child", "_mapping-child"):
        ...
        try:
            card_index = int(request["selected_gpu"]["physical_visible_index"])
        except (KeyError, TypeError, ValueError) as exc:
            raise S10R4Error("correctness worker selected-GPU binding is malformed") from exc
        expected_environment = _worker_environment(card_index=card_index)
    if request.get("environment") != expected_environment:
        raise S10R4Error("formal worker request environment differs from the frozen worker environment")
    if dict(os.environ) != expected_environment:
        raise S10R4Error("formal worker complete process environment mismatch")
...
def command_internal_worker(arguments: argparse.Namespace) -> int:
    if not Path(arguments.request).is_absolute() or not Path(arguments.result).is_absolute():
        raise S10R4Error("formal worker request/result paths must be absolute")
    ...
    if (
        request.get("cwd") != request_path.parent.as_posix()
        or Path.cwd().resolve(strict=True) != request_path.parent
    ):
    ...
    lock_digest = request.get("effective_lock_digest")
    if not isinstance(lock_digest, str) or re.fullmatch(r"[0-9a-f]{64}", lock_digest) is None:
        raise S10R4Error("formal worker effective-lock digest is malformed")
    handlers = {
    ...
    if handler is None:
        raise S10R4Error("formal worker handler dispatch is unknown")
    _admit_current_argv(arguments)
    return int(handler(request, result_path))
