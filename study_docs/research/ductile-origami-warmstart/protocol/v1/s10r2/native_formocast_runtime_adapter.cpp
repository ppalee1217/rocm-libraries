...
// S10R2's single native conformance path.  The helper intentionally constructs
// real TensileLite solutions, calls AllSolutionsIterator::preProblem (therefore
// the pinned checkSolution -> Formocast -> stable queue path), and separately
// records the same native Formocast predictions so the queue transcript is
// independently checkable without substituting a Python implementation.
...
namespace s10r2_native
{
    ...
    static std::shared_ptr<ContractionSolution> solution(CohortRow const& row)
    {
        ...
        out.macroTile     = TensileLite::dim3(row.mapping.macroTile[0],
    ...
    static ContractionProblemGemm problem(CohortRow const& row)
    {
        ...
        size_t ldc = row.M;
        size_t ldd = row.M;
        auto value = ContractionProblemGemm::GEMM_Strides(false,
                                                         false,
    ...
    static double predict(CohortRow const& row)
    {
        ...
        if(!std::isfinite(prediction.microSeconds) || !std::isfinite(prediction.hitRate))
        ...
        runtimeQueue(std::vector<CohortRow> const& rows, double threshold)
    {
        ...
        hardware->analyticalHardware = std::make_shared<origami::hardware_t>(
            architecture,
            304,
            64 * 1024,
        ...
        while(iterator.moreSolutionsInProblem())
        {
            ...
            if(!current)
                throw std::runtime_error("runtime queue returned an absent solution");
    ...
    static void writeIntVector(std::ostream& output, std::vector<int> const& values)
    {
        output << '[';
    ...
    int evaluate_formocast_all_solutions_cohort(std::istream& input, std::ostream& output)
    {
        size_t rowCount = 0;
        size_t thresholdCount = 0;
        if(!(input >> rowCount >> thresholdCount) || rowCount == 0 || thresholdCount == 0)
            throw std::runtime_error("invalid S10R2 cohort header");
        ...
        for(size_t index = 0; index < thresholdCount; ++index)
        {
            ...
            if(!(input >> token >> value) || value < 0)
                throw std::runtime_error("invalid S10R2 threshold");
        ...
        for(size_t index = 0; index < rowCount; ++index)
        {
            ...
            if(!(input >> row.ordinal >> valid >> row.M >> row.N >> row.batch >> row.K
                ...
                throw std::runtime_error("malformed S10R2 cohort row");
            if(row.ordinal != static_cast<int>(index))
                throw std::runtime_error("S10R2 cohort ordinals must be contiguous");
        ...
        if(performance.empty())
            throw std::runtime_error("S10R2 cohort has no valid solutions");
        ...
        output << "{\"schema_version\":1,"
               ...
               << "\"derived-plr-zero\"],"
               << "\"queues\":{";
        for(size_t thresholdNumber = 0; thresholdNumber < thresholds.size(); ++thresholdNumber)
        {
            if(thresholdNumber)
                output << ',';
            auto const& token = thresholds[thresholdNumber].first;
            double threshold = thresholds[thresholdNumber].second;
            auto actualQueue = runtimeQueue(rows, threshold);
            ...
            {
                for(auto const& value : performance)
                    sortedOrdinals.push_back(value.first);
                thresholdIndex
            ...
            output << '"' << token << "\":{"
                   << "\"prediction_disabled\":" << (threshold > 1.0 ? "true" : "false")
                   << ",\"sorted_ordinals\":";
            ...
            output << ",\"queue_prefix\":";
            writeIntVector(output, actualQueue);
            output << '}';
        }
        output << "}}\n";
        return 0;
    ...
    try
    {
        ...
        for(int index = 1; index < argc; ++index)
        {
            ...
            if(argument == "--input" && index + 1 < argc)
                ...
                throw std::runtime_error("usage: s10r2-native --input INPUT --output OUTPUT");
        }
        std::ifstream input(inputPath);
        std::ofstream output(outputPath, std::ios::trunc);
        if(!input || !output)
            throw std::runtime_error("cannot open S10R2 helper input/output");
        int result
            = s10r2_native::evaluate_formocast_all_solutions_cohort(input, output);
        input >> std::ws;
        if(!input.eof() || !output)
            throw std::runtime_error("S10R2 helper stream failure");
        return result;
    }
    catch(std::exception const& error)
    {
        std::cerr << "s10r2-native: " << error.what() << '\n';
        return 2;
    }
}
