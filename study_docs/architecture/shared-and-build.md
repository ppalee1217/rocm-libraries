...
| 子資料夾 | 用途 |
|----------|------|
| `include/`、`src/` | C++ IR 與 pass 實作 |
| `python_module/` | nanobind Python 綁定 |
| `hardware/` | 各架構的指令定義（TableGen 風格 `.def`） |
| `tests/`、`tools/`、`examples/`、`docs/` | 測試 / 工具（含 `stinkytofu-check`）/ 範例 / 文件 |
...
- `shared/tensile`、`shared/rocroller` — 本機僅骨架（少量檔）。
  > 注意：hipBLASLt **自帶**一份 in-repo fork 在 `projects/hipblaslt/tensilelite/`（那份才是完整、會用到的，見 [hipblaslt-layout.md](hipblaslt-layout.md)）。

## `dnn-providers/`：DNN provider 整合層

| 子項 | 內容 |
|------|------|
| `cmake/` | provider 用的 CMake 輔助（版本檢查、ClangTidy、Sanitizers、Tests 等） |
| `hip-kernel-provider/` | HIP kernel provider（`src/`） |
| `miopen-provider/` | MIOpen provider（`cmake/` 等） |
| `integration-tests/` | 跨 provider 整合測試（`cmake/`、`gpu_ref/`） |

## `cmake/`：superbuild 共用建置設定

| 子資料夾 | 內容 |
|----------|------|
| `modules/` | 共用 CMake module：`fetch_rocm_cmake.cmake`、`default_amdclang.cmake`、`shared_third_party.cmake`、`add_subdirectory_with_message.cmake`、`ClangTidy.cmake`、`CheckToolVersion.cmake` |
| `toolchains/` | 編譯器 toolchain：`linux-amdclang.cmake`、`rocm-clang.cmake` |
...
> `shared/` 有三個真有內容的共用依賴（stinkytofu / origami / mxdatagenerator）；`cmake/`、`dnn-providers/` 是建置與整合骨架；
> `projects/` 其餘 22 個庫在本機只是 build 骨架。
